...
bool check(int L, int R)
{
        for(int i = 0; i < n; i++) {
                for(int j = 0; j < n; j++) {
                        ok[i][j] = (i == j) || D[i*n+j] >= L && D[i*n+j] <= R;
                }
        }
...
int HardProof:: minimumCost( vector <int> _D )
{
        n = (int)sqrt(_D.size() * 1.0 + 0.1);
        D = _D;
        sort(_D.begin(), _D.end());
        int p = 0;
        int ret = -1;
        for(int i = 0; i < _D.size(); i++) {
                while(p < _D.size() && !check(_D[i], _D[p])) {
                        p++;
                }
                if(p == _D.size()) break;
                if(ret == -1 || _D[p] - _D[i] < ret) ret = _D[p] - _D[i];
        }
        return ret;
} 
// BEGIN CUT HERE
